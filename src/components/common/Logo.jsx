...
export function SentinelMark({ dark = false }) {
  return (
    <div className="hidden items-center gap-2 border-l border-ink-200 pl-3 sm:flex" style={dark ? { borderColor: "rgba(255,255,255,0.15)" } : undefined}>
      <div className="leading-tight">
        <p className={`font-display text-sm font-bold ${dark ? "text-white" : "text-navy-800"}`}>MPLADS Sentinel</p>
        <p className={`text-[11px] ${dark ? "text-white/60" : "text-ink-500"}`}>AI Monitoring &amp; Risk Intelligence</p>
      </div>
    </div>
  );
}
